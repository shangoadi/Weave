	...
	public class BLGTree
	{
		...
		public function getPointVector(minImportance:Number = 0, visibleBounds:IBounds2D = null):Vector.<BLGNode>
		{
				...
				while (stackPos >= 0)
				{
					...
					else // OP_VISIT
					{
						...
						if (visibleBounds != null)
						{
							...
							{
								if (gridID == 0) // on screen
								{
									visible = true;
								}
								else if (sameOutsideGridID)
								{
									resultCount--; // drop previous node
								}
								else
								{
									sameOutsideGridID = true;
								}
							}
							else // gridID changed
							{
								...
								if (sameOutsideGridID && Math.abs(gridID - prevGridID) % 6 == 1)
									resultCount--; // drop previous node
								prevGridID = gridID;
								sameOutsideGridID = false;
							}
						}
			...
			*/
			...
			// if there was nothing on screen and nothing passed a gridline, don't return anything
			if (!visible)
